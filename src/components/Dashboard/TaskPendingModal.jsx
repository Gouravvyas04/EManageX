...
const TaskPendingModal = ({ onClose }) => {
  ...
  useEffect(() => {
    const modalShown = localStorage.getItem('taskPendingModalShown');
    if (!modalShown) {
      setIsVisible(true);
      localStorage.setItem('taskPendingModalShown', 'true'); // Mark modal as shown
    }
  }, []);

  if (!isVisible) return null; // Do not render modal if already shown
